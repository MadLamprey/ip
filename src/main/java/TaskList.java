...
public class TaskList {
    ...
    public void addEvent(String task) throws BrunoException {
        ...
        if (!task.substring(task.indexOf("event") + 1).contains("/from")
                || !task.substring(task.indexOf("event") + 1).contains("/to")) {
            System.out.print("\t");
            throw new BrunoMissingEventException();
        }
        list.add(new Event(task.substring(task.indexOf(' ') + 1, task.indexOf('/') - 1),
                task.substring(task.indexOf("from")
                        + 5, task.lastIndexOf('/') - 1), task.substring(task.indexOf("to") + 3)));
        String s = "\tWoof. I have added this task:\n\t\t" + list.get(list.size() - 1).getString();
        ui.displayMessage(s);
        storage.writeToFile();
    }
    ...
    public void markTask(String task) throws BrunoException {
        ...
        String s = "\tWoof Woof! I have marked the task as done.\n\t"
    ...
    public void displayListSum() {
        String s = "\tNow you have " + list.size() + (list.size() == 1 ? " task" : " tasks") + " in your list.";
        ui.displayMessage(s);
    }
        ...
        for (Task t : list) {
            if (t instanceof Deadline) {
                ...
                if ((d.isAfter(((Event) t).from.toLocalDate()) && d.isBefore(((Event) t).by.toLocalDate()))
                        || d.isEqual(((Event) t).from.toLocalDate()) || d.isEqual(((Event) t).by.toLocalDate())) {
                    s = s + "\t" + (++counter) + ". " + t.getString() + "\n";
                }
            }
        }
